...
<body>
  ...
  if (isset($_GET['cliente']))
    echo "<div class='alert alert-danger' role='alert' style='padding-left: 5vw;'>
                Não foi possível realizar o pedido pois <strong>nenhum</strong> cliente foi selecionado.
            </div>";
  if (isset($_GET['produto']))
    echo "<div class='alert alert-danger' role='alert' style='padding-left: 5vw;'>
                Não foi possível realizar o pedido pois <strong>nenhum</strong> produto foi selecionado.
            </div>";
  ?>
      ...
      <tbody>
        ...
        if ($resultado) {
          while ($registros = mysqli_fetch_array($resultado, MYSQLI_ASSOC)) {
            ...
            $valor= strval($valor);
            if (strpos($valor, '.'))
              echo '<td>R$' . str_replace('.', ',', $valor) . '</td>';
            else
